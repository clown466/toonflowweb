<template>
  <VueFlow
    ...
    <template #node-workbench="props">
      <workbench :id="props.id" v-model="workbenchNodeData" :handleIds="props.data.handleIds" />
    </template>
    ...
    <div class="floatingWindow">
      <div class="episodesSelect f ac" v-if="showEpisodeSelector">
        <t-select
          :value="episodesId"
      ...
      <div class="openRightChatBoxBtn c" v-if="showRightChat" v-show="!openShowVisible" @click.stop="openShowVisible = true">
        <i-menu-unfold-one theme="outline" size="24" />
      </div>
      <transition name="slide" v-show="openShowVisible" v-if="showRightChat && episodesId">
        <rightChatBox :title="title" v-model="flowData" @close="openShowVisible = false" />
      </transition>
    </div>
    <t-guide v-if="!embedded && showEpisodeSelector" v-model="current" :steps="steps" @finish="() => (current = -1)" />
    <t-tag variant="outline" class="fps" v-if="showRightChat && !openShowVisible">{{ fps }}</t-tag>
  </VueFlow>
</template>
...
const props = withDefaults(
  defineProps<{
    ...
    autoSelectFirstEpisode?: boolean;
  }>(),
  {
    flowId: "mainFlowBox",
    embedded: false,
    ...
    showRightChat: true,
    autoSelectFirstEpisode: true,
  },
);

const flowId = computed(() => props.flowId);
const embedded = computed(() => props.embedded);
const showEpisodeSelector = computed(() => props.showEpisodeSelector);
const showRightChat = computed(() => props.showRightChat);
const openShowVisible = ref(props.showRightChat);
const {
  toObject,
  fromObject,
  fitView,
...
const { layout } = useLayout(props.flowId);
...
const workbenchNodeData = computed<any>({
  get: () => flowData.value.workbench,
  set: (value) => {
    flowData.value.workbench = value;
  },
});
...
async function getScriptData() {
  ...
  if (props.autoSelectFirstEpisode && episodesOptions.value.length && !episodesId.value) {
...
const current = useLocalStorage(`productionCurrent-${props.flowId}`, 0);
const steps = [
  {
